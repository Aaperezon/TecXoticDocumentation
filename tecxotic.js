    ...
    this.onResize();window.addEventListener('resize', this.onResize, false);var edges = new THREE.EdgesGeometry(model.children[0].geometry);let line = new THREE.LineSegments(edges);line.material.depthTest = false;line.material.opacity = 0.5;line.material.transparent = true;line.position.x = 0.5;line.position.z = -1;line.position.y = 0.2;this.modelGroup = new THREE.Group();model.layers.set(0);line.layers.set(1);this.modelGroup.add(model);this.modelGroup.add(line);this.scene.add(this.modelGroup);}}function loadModel()
{
  ...
  loader.load('src_models/Rov.obj', function (obj) {
    object = obj;
  });
...
function setupAnimation(model)
{
  ...
  gsap.set(kolop.rotation, { y: tau * -.25 });
  gsap.set(kolop.position, { x: 80, y: 0, z: -60 });
  gsap.set(kolop.scale, { x: 6, y: 6, z: 6});
  ...
  tl.to(kolop.rotation, { x: 0, z: 0, y: tau * .25 }, delay);
  tl.to(kolop.position, { x: 0, y: -10, z: 50 }, delay);

  delay += sectionDuration;
  delay += sectionDuration;

  tl.to(kolop.rotation, { x: tau * 0.25, y: tau * .5, z: 0, ease: 'power4.inOut' }, delay);
  tl.to(kolop.position, { z: 30, ease: 'power4.inOut' }, delay);

  delay += sectionDuration;

  tl.to(kolop.rotation, { x: tau * 0.25, y: tau * .5, z: 0, ease: 'power4.inOut' }, delay);
  tl.to(kolop.position, { z: 60, x: 30, ease: 'power4.inOut' }, delay);

  delay += sectionDuration;

  tl.to(kolop.rotation, { x: tau * 0.35, y: tau * .75, z: tau * 0.6, ease: 'power4.inOut' }, delay);
  tl.to(kolop.position, { z: 100, x: 20, y: 0, ease: 'power4.inOut' }, delay);

  delay += sectionDuration;

  tl.to(kolop.rotation, { x: tau * 0.15, y: tau * .85, z: -tau * 0, ease: 'power1.in' }, delay);
  tl.to(kolop.position, { z: -150, x: 0, y: 0, ease: 'power1.inOut' }, delay);

  delay += sectionDuration;

  tl.to(kolop.rotation, { duration: sectionDuration, x: -tau * 0.05, y: tau, z: -tau * 0.1, ease: 'none' }, delay);
  tl.to(kolop.position, { duration: sectionDuration, x: 0, y: 30, z: 320, ease: 'power1.in' }, delay);
